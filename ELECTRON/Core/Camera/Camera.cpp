...
Core::Camera::Camera(glm::vec3 position) : Front(glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(SPEED), MouseSensitivity(SENSITIVITY), Zoom(ZOOM)
{
    Position = position;
...
glm::vec3 Core::Camera::GetCameraPos()
{
    return Position;
}
glm::vec3 Core::Camera::GetCameraFront()
{
    return Front;
}
...
void Core::Camera::ProcessMouseMovement(float xoffset, float yoffset, GLboolean constrainPitch)
{
